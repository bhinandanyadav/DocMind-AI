...
export default function Study() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-5 lg:p-8">
      <div className="mb-8">
        <p className="text-sm font-semibold uppercase tracking-wide text-indigo-600 mb-2">Study Mode</p>
        <h1 className="text-3xl font-bold text-gray-900">Study Material</h1>
        <p className="mt-2 text-gray-500">Select a document to generate notes, MCQs, questions, and flashcards</p>
      </div>
      ...
      {!isLoading && documents.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-500 text-sm max-w-md mb-4">Upload and process a document first, then come back to generate study material.</p>
      ...
      {!isLoading && documents.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {documents.map((doc) => (
            <Link key={doc.id} to={`/documents/${doc.id}/study`}>
